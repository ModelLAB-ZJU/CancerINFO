var tree = (function () {
    ...
    function process_children(parentNode, childData) {
		...
		//childNode.tissue = childData.tissue;
		//childNode.level = childData.level;
    ...
    function initDataAndTree(version) {
        var jsonUrl = '../data/oncotree.json?';
        ...
        var nodeText = nodeEnter.append("svg:text")
            ...
            .text(function (d) {
                ...
                _qtipContent += '<b>Name:</b> ' + d.name.replace(/\(\w+\)/gi, '') + '<br/>';
						//_qtipContent += '<b>Name:</b> <a class="qtip-link" href="cancer_browser.php?tissue=' + d.tissue + '">' + d.name.replace(/\(\w+\)/gi, '') + '</a><br/>';
					//}
					//else if(d.level===2){
						//_qtipContent += '<b>Name:</b> <a class="qtip-link" href="cancer_browser.php?tissue=' + d.tissue + '&cancer='+ d.name.replace(/\(\w+\)/gi, '') +'">' + d.name.replace(/\(\w+\)/gi, '') + '</a><br/>';
					//}
					//else{
						//_qtipContent += '<b>Name:</b> <a class="qtip-link" href="cancer_browser.php?tissue=' + d.tissue + '&cancer='+ d.name +'">' + d.name.replace(/\(\w+\)/gi, '') + '</a><br/>';
					//}
