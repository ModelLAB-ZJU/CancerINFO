...
function gene_info(){
	require_once("mysql.php");
		...
		while($t = $db->fetch_array()){
			...
			$gene[$t['element_symbol']]['gene_alteration_in_cancer_therapy'] = "No";
			if(!empty($t['element_cancer_oncotree_gene_alteration_drug'])){
				$gene[$t['element_symbol']]['gene_alteration_in_cancer_therapy'] = "Yes";
			}
		}
		$geneinfo = array();
		$db -> query("SELECT * FROM gene_info");
		...
		foreach($genes as $g){
			  ...
			  $gene_info['gene_alteration_in_cancer_therapy'] = $gene[$g]['gene_alteration_in_cancer_therapy'];
